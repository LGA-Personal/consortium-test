...
from exo.consortium.verification.canonicalizer import verify_commitment
...
class CoordinatorServicer(consortium_pb2_grpc.CoordinatorServiceServicer):
    ...
    def SubmitLayerCommitments(
        self,
        request: consortium_pb2.SubmitLayerCommitmentsRequest,
        context: grpc.ServicerContext,
    ) -> consortium_pb2.SubmitLayerCommitmentsResponse:
        """Receive layer commitments from EXO integration."""
        session_id = request.session_id
        command_id = request.command_id
        token_index = request.token_index
        device_rank = request.device_rank

        # Parse commitments from protobuf
        commitments = {
            lc.layer_idx: lc.commitment for lc in request.commitments
        }

        logger.info(
            f"Received {len(commitments)} layer commitments for token {token_index} "
            f"from device {device_rank} (session: {session_id}, command: {command_id})"
        )

        # Store commitments in session state if session exists
        if session_id and session_id in self.state.sessions:
            session = self.state.sessions[session_id]

            # Use proper Session method to store commitments
            session.add_layer_commitments(
            ...
            # Select layers for audit based on audit_probability
            layers_to_audit = session.select_layers_for_audit(token_index)

            if layers_to_audit:
                logger.info(
                    f"Selected {len(layers_to_audit)} layers for audit "
                    f"(token {token_index})"
                )
                # Schedule audits (verifier selection would happen here)
                # For now, just log - actual audit dispatch is M5 completion
                for dev_rank, layer_idx in layers_to_audit:
                    logger.debug(
                        f"  -> Audit: device={dev_rank}, layer={layer_idx}"
                    )

        return consortium_pb2.SubmitLayerCommitmentsResponse(
            acknowledged=True,
            error="",
        )
