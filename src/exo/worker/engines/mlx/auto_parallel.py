...
# Type alias for verification callback (consortium integration)
VerificationCallback = Callable[[int, mx.array], None] | None
...
def pipeline_auto_parallel(
    ...
    Args:
        ...
        verification_callback: Optional callback for consortium verification.
            If provided, each layer is wrapped with VerifiedLayer that invokes
            the callback with (layer_idx, hidden_state) after each forward pass.
    ...
    # Consortium integration: wrap layers with verification if callback provided
    if verification_callback is not None:
        try:
            from exo.consortium.exo_integration.verified_layer import VerifiedLayer
            layers = [
                VerifiedLayer(layer, layer_idx=start_layer + i, callback=verification_callback)
                for i, layer in enumerate(layers)
            ]
        except ImportError:
            # Consortium not installed, skip verification
            pass
