...
def load_mlx_items(
    ...
    """
    Load MLX model, tokenizer, and sampler.

    Args:
        bound_instance: The bound instance with shard metadata
        group: Distributed group (None for single device)
        verification_callback: Optional callback for consortium verification.
            If provided, each layer is wrapped to invoke the callback with
            (layer_idx, hidden_state) after each forward pass.
    ...
    if group is None:
        ...
        # Apply verification wrapping for single-device mode
        if verification_callback is not None:
            try:
                from exo.consortium.exo_integration.verified_layer import VerifiedLayer
                # Get the inner model and its layers
                inner_model = model.model if hasattr(model, "model") else model
                if hasattr(inner_model, "layers"):
                    original_layers = list(inner_model.layers)
                    wrapped_layers = [
                        VerifiedLayer(layer, layer_idx=i, callback=verification_callback)
                        for i, layer in enumerate(original_layers)
                    ]
                    inner_model.layers = wrapped_layers
                    logger.info(f"Wrapped {len(wrapped_layers)} layers with verification (single-device mode)")
            except ImportError:
                logger.warning("Could not import consortium verification - skipping layer wrapping")
...
def shard_and_load(
    ...
    """
    Shard and load model for distributed inference.

    Args:
        shard_metadata: Metadata describing the shard configuration
        group: Distributed group for communication
        verification_callback: Optional callback for consortium verification.
            Only used for pipeline parallelism.
    ...
    match shard_metadata:
        ...
        case PipelineShardMetadata():
            ...
            # Pass verification callback for consortium integration
            model = pipeline_auto_parallel(
                model, group, shard_metadata, verification_callback
            )
