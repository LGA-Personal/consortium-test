...
class LayerCommitmentsGenerated(BaseEvent):
    """
    Commitments from verified layer execution.

    Emitted after each token generation when verification is enabled.
    Used by consortium for auditing distributed inference.
    """
    command_id: CommandId
    token_index: int
    commitments: dict[int, str]  # layer_idx -> 64-char hex-encoded SHA-256
    device_rank: int
    timestamp_ms: int
