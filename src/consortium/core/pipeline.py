...
from consortium.identity.signing import create_and_sign_receipt
from consortium.verification.canonicalizer import compute_commitment
from consortium.worker.kv_cache import StageKVCache
from consortium.worker.model_shard import ForwardResult, ModelShard, ShardConfig
...
class InProcessPipeline:
    ...
    def execute_token(
        ...
        # Initial activation (mock: random embedding)
        if initial_embedding is None:
            current_activation = np.random.randn(
